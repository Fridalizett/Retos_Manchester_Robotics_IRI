...
class path_g:
    def __init__(self):
        ...
        self.listax=rospy.get_param("list_of_x",[2.0, 2.0, 0.0, 1.0])
        self.listay=rospy.get_param("list_of_y",[2.0, 2.0, 1.0, 0.0])
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.first:
                ...
                self.pose.theta += dt * self.r * ((self.wr - self.wl) / self.l)
                ...
                #erorres
                self.error.eang = np.arctan2(self.listay[self.i]-self.pose.y, self.listax[self.i]-self.pose.x) - self.pose.theta
                self.error.ed = 0#np.sqrt(np.square(self.listax[self.i]-self.pose.x) + np.square(self.listay[self.i]-self.pose.y))
                # Publicar las posiciones y el error
                if self.error.ed < 0.1:
                    self.error.ed=0
                if self.error.eang < 0.1 and self.error.eang > -0.1:
                    self.error.eang=0
                if self.error.eang==0 and self.error.ed==0:
                    self.i=0
